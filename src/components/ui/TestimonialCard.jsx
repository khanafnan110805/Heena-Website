...
export default function TestimonialCard({ testimonial }) {
  return (
    <div className="relative bg-bg-card border border-border-dark rounded-sm p-6 md:p-7 flex flex-col gap-4">
      <span className="absolute top-4 right-6 font-display text-7xl text-gold-muted/30 leading-none select-none" aria-hidden="true">"</span>
      ...
      <div className="pt-2 border-t border-border-dark">
        ...
        {testimonial.designDescription && (
          <p className="font-body text-xs text-text-muted mt-1 italic">{testimonial.designDescription}</p>
        )}
      </div>
    </div>
  )
