...
export default function ProductCard({ product }) {
  return (
    <div className="group bg-bg-card border border-border-dark rounded-sm overflow-hidden hover:border-gold-muted transition-all duration-300">
      ...
      <div className="p-5 space-y-3">
        <div>
          <h3 className="font-body text-text-primary font-medium text-base leading-snug">{product.name}</h3>
          <p className="font-body text-text-tertiary text-sm mt-1 line-clamp-2">{product.description}</p>
        </div>
        <ul className="space-y-1">
          {product.features.slice(0, 3).map((f, i) => (
            <li key={i} className="font-body text-xs text-text-muted flex items-center gap-1.5">
              <span className="text-gold-muted">·</span>{f}
            </li>
          ))}
        </ul>
        <div className="flex items-center justify-between pt-2 border-t border-border-dark">
          ...
          <WhatsAppCTA message={product.whatsappMessage} label="Order" size="sm" variant="outlined" />
        </div>
      </div>
    </div>
  )
