...
export default function ServiceCard({ service, compact = false }) {
  const galleryRoute = `/gallery/${service.category}`

  if (compact) {
    return (
      <Link to={galleryRoute} className="group block bg-bg-card border border-border-dark rounded-sm overflow-hidden hover:border-gold-muted transition-all duration-300">
        <div className="relative h-48 overflow-hidden">
          <img src={service.imageUrl} alt={service.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
          <div className="absolute inset-0 bg-hero-overlay opacity-40 group-hover:opacity-60 transition-opacity duration-300" />
        </div>
        <div className="p-5">
          <h3 className="font-display text-xl text-gold-champagne font-light mb-1">{service.title}</h3>
          <p className="font-body text-sm text-text-tertiary line-clamp-2">{service.subtitle}</p>
  ...
  return (
    <div className="bg-bg-card border border-border-dark rounded-sm overflow-hidden">
      <div className="relative h-64 overflow-hidden">
        <img src={service.imageUrl} alt={service.title} className="w-full h-full object-cover" loading="lazy" />
