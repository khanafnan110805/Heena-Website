...
import { DecorativeCorner } from '../../atoms/DecorativeRule'
...
export function PackageCard({ pkg }: PackageCardProps) {
  ...
  return (
    <div
      className={`
        relative flex flex-col bg-bg-card border rounded-sm overflow-hidden
        transition-all duration-300
        ${isFeatured
          ? 'border-gold-primary shadow-[0_0_40px_rgba(201,168,76,0.12)]'
          : 'border-border-dark hover:border-gold-muted'
        }
      `}
    >
      {isFeatured && <DecorativeCorner />}

      {isFeatured && (
        <div className="absolute top-4 right-4 z-10">
          <span className="font-body text-xs text-bg-base bg-gold-primary px-2.5 py-1 tracking-widest uppercase">
            Most Popular
          </span>
        </div>
      )}
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-5">
          <p className="font-body text-xs text-gold-primary uppercase tracking-widest mb-1">
            {pkg.tier}
          </p>
